...
const FIELDS = ['标题', '链接', '封面', '作者', '视频时长', '点赞数', '收藏数', '评论数', '分享数', '正文', '采集时间', '失败原因'];
const FIELD_SCHEMA = [
  { name: '标题', type: 'text' }, { name: '链接', type: 'url' }, { name: '封面', type: 'url' }, { name: '作者', type: 'text' },
  { name: '视频时长', type: 'text' },
  ...
  { name: '正文', type: 'text' }, { name: '采集时间', type: 'text' }, { name: '失败原因', type: 'text' },
];
...
async function configure(args = {}) {
  ...
  if (args.bind) {
    ...
    return json({ ok: true, action: 'bound', path: configPath(), base: config.feishu.video });
  }
  if (args.create || args['create-test']) {
    ...
    if (existing && validateConfig(existing).length === 0) return json({ ok: true, action: 'already_configured', path: configPath(), base: existing.feishu.video });
    ...
    const base = createBase({ name: `小红书视频采集试用库${suffix}`, fields: FIELD_SCHEMA, identity, timezone: existing?.timezone || 'Asia/Shanghai', tableName: '视频采集库' });
    const config = normalizeConfig({ initialized: true, feishu: { identity, video: base } });
    writeJsonAtomic(configPath(), config);
  ...
  for (const url of urls) {
    try {
      ...
      if (normalized.type !== 'video') {
        items.push({ status: 'failed', original_url: url, final_url: page.finalUrl, note_id: noteId, reason: '视频 Lite 版只支持视频笔记；图文笔记请使用图文 Lite 或 Pro 版' });
        continue;
      }
      delete normalized.data.transcript;
      delete normalized.data.subtitle_url;
      items.push({ ...normalized, status: 'success', collected_at: new Date().toISOString(), analysis: {} });
    } catch (error) {
...
async function archive(args = {}) {
  ...
  const success = items.filter((item) => item.status === 'success' && item.type === 'video');
  const failed = items.filter((item) => item.status !== 'success');
  const outputDir = args['output-dir'] ? path.resolve(args['output-dir']) : OUTPUT_DIR;
  fs.mkdirSync(outputDir, { recursive: true });
  const backup = path.join(outputDir, `xhs-video-lite-${new Date().toISOString().replace(/[:.]/g, '-')}.xlsx`);
  writeXlsx(backup, { 视频采集: [FIELDS, ...success.map(rowFor)], 失败记录: [FIELDS, ...failed.map(failureRow)] });
  ...
  const binding = loaded.config.feishu.video;
...
try {
  ...
  else throw new Error('用法：xhs-video-lite.mjs <doctor|configure|extract|archive> [参数]');
} catch (error) {
