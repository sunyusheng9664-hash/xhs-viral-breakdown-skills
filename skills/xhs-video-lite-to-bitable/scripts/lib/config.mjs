...
export const APP_DIR = 'xhs-video-lite';
...
export function configPath({ home = os.homedir(), env = process.env, platform = process.platform } = {}) {
  if (env.XHS_VIDEO_LITE_CONFIG_HOME) return path.join(env.XHS_VIDEO_LITE_CONFIG_HOME, 'config.json');
...
function normalizeBinding(value = {}) {
  return {
    base_name: value.base_name || '小红书视频采集试用库',
    base_token: value.base_token || '',
    base_url: value.base_url || '',
    table_name: value.table_name || '视频采集库',
...
export function normalizeConfig(input = {}) {
  const source = input.feishu?.video || input.feishu || input.video || input;
...
export function validateConfig(config, { requireBinding = true } = {}) {
  ...
  const binding = config.feishu?.video;
  if (!binding) errors.push('缺少 feishu.video');
  if (requireBinding) {
    for (const key of ['base_token', 'table_id', 'view_id']) {
      if (!binding?.[key]) errors.push(`缺少 feishu.video.${key}`);
    }
  }
...
export function loadConfig({ migrate = true, ...options } = {}) {
  ...
  if (migrate) {
    ...
    if (fs.existsSync(pro)) {
      ...
      const config = normalizeConfig({ feishu: { identity: proConfig.feishu?.identity || 'bot', video: proConfig.feishu?.video } });
