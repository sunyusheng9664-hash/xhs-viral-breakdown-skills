...
export function createBase({ name, fields, identity = 'bot', timezone = 'Asia/Shanghai' }) {
  const result = runLark(['base', '+base-create', '--as', identity, '--name', name, '--time-zone', timezone, '--table-name', '内容拆解库', '--fields', JSON.stringify(fields), '--format', 'json']);
